$(document).ready(function()
{
	...
	function editclick()
	{
		...
		$('textarea.tinymce').tinymce({
			...
			script_url : jspath + 'tiny_mce/tiny_mce.js',
	...
	{
		targetcontents = $('#targetpage').contents();
		//targetcontents.find('head').append(styleinclude);
		
		//For each element with bac-id
        targetcontents.find('*[data-bac-id]').each(function(i) {
	...
	$('#targetpage').load(function(){
        $('#targetpage').contents().find('body').before('<style>.bac-hoveroverlay{border:2px solid red;}.liveeditor{}</style>');
        ...
        //$('#targetpage').contents().find('body').before('<script type="text/javascript">alert("hi!");</script>');
